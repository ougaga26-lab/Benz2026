...
const origin='https://benz2026-api.ougaga26.workers.dev';
const env=Object.fromEntries(fs.readFileSync('.env','utf8').trim().split(/\r?\n/).map(l=>{const i=l.indexOf('=');return [l.slice(0,i),l.slice(i+1)]}));
function client(){let cookie='';return async(path,body)=>{const r=await fetch(origin+path,{method:body?'POST':'GET',headers:{'Content-Type':'application/json',Origin:'https://ougaga26-lab.github.io',Authorization:cookie?'Bearer '+cookie:''},...(body?{body:JSON.stringify(body)}:{})});if(r.headers.get('X-Session'))cookie=r.headers.get('X-Session');return {status:r.status,data:await r.json()};};}
...
const parts=reward.data.token.split('.');parts[2]=(Date.now()-120000).toString(36);const raw=parts.slice(0,3).join('.');const expired=raw+'.'+createHmac('sha256',env.EVENT_SECRET).update('qr:'+raw).digest('hex').slice(0,32);assert.equal((await staff('/api/staff',{action:'confirm',token:expired,confirm:true})).status,410);
